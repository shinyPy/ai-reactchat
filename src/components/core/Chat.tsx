...
const Chat: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (apiKey) {
      ChatService.getModels(apiKey, userSettings.openaiEndpoint)
        .then(models => {
          setModels(models);
        })
        .catch(err => {
          if (err instanceof CustomError && err.responseJson === 'INVALID_ENDPOINT') {
            NotificationService.handleError('Invalid endpoint or network error', 'Failed to get list of models');
          } else {
            NotificationService.handleUnexpectedError(err, 'Failed to get list of models');
          }
        });
    }
  }, [userSettings.apiKey, userSettings.openaiEndpoint]);
